...
### Planned, not built
- **30-day history — planned 09-26, ON HOLD** (his words: "planned out but not executed until we can find the reason why the Barn Office HubDuo is quasi-crashing so soon after an upload"). Do not build it until that is explained; the Pi/network session is watching `~/netwatch.log` on the Pi.
  - *What he gets:* the rolling 24 h stays as it is; a day picker opens any of the last 30 days, **one day at a time**, from the same hour files.
  - *Why one day at a time:* a day is ~29 MB of hour files and ~50 MB of browser memory; 7 days at once would be ~200 MB / ~350 MB, the kind of load that crashed the May 1 map on iOS.
  - *Upload cost: close to nothing extra.* Since 09-26 each hour file is built once, 10 min after its hour, and pushed once (PR #34). Keeping 30 days means not deleting them from the `live-data` tree, not sending them again. The one new cost: every push also sends the changed directory listings, so files go in per-day folders (`h/20260926/15.json`) and a push touches only today's small folder, not a 720-entry one. Measure push sizes with vnstat before and after, since this is exactly what the HubDuo question is about.
  - *Pi:* `publish.py` keeps frozen files for 30 days and removes older ones from the tree; memory unchanged (frozen hours are read from disk, not rebuilt). Retention in hours becomes a setting. Needs an installer run.
  - *GitHub size:* ~17 MB/day compressed (h + t), so ~0.5 GB for 30 days, under GitHub's ~1 GB guidance. Force-pushes leave old objects for GitHub to clean up; watch the repo size in Settings after the first week.
  - *Page:* `live.json` gains a list of available days; the picker loads that day's `h/` files (fingerprint-checked as now) and switches the timeline to that day; "Now" returns to the rolling 24 h. Events and farm passes per day come from the Pi's `events/*.jsonl`, which would need publishing per day too (small). `flights.html` gets it through `make_public.py` as usual.
  - *Open for him when it's built:* 30 days is my suggestion, not his decision; the Pi holds months.
